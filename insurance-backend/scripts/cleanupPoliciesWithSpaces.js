/**
 * Cleanup script to remove policies with spaces in policy_number
 * Assumption: Duplicates without spaces already exist in the database
 */
...
async function cleanup() {
  try {
    console.log('Starting cleanup of policies with spaces in policy_number...');
    ...
    console.log(`Found ${recordsToDelete} policies with spaces in policy_number`);

    if (recordsToDelete === 0) {
      console.log('No policies with spaces found. Cleanup complete.');
      await pool.end();
      process.exit(0);
    }
    ...
    // Delete records with spaces
    const [deleteResult] = await pool.query(
      `DELETE FROM policies WHERE policy_number LIKE '% %'`
    );

    console.log(`\nSuccessfully deleted ${deleteResult.affectedRows} policies with spaces in policy_number.`);
